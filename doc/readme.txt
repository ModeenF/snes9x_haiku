...
-Yellow means the ROM has probably been altered. Either it's a translation, 
 PD ROM, hacked, or possibly a bad ROM. It may also be an overdumped ROM.
-Red means the ROM the ROM is definitely hacked and that a new version should be 
found. some ROM Tools such as NSRT can also fix these ROMs.
...
pressing more then one or 2 buttons at a time. First you must hit "toggle 
diagonals" so that you are able to change them.
...
Load your game and select the controller using 7. If the game does not pick up 
the controller, reset the game. At some point, this will probably be rewritten.
...
usually read-only! You can change the folder where Snes9X saves and loads S-RAM 
and freeze-files using the Settings Dialog, available from the Options menu.
...
Netplay Support
===============
Netplay Support is disabled in this release
...
- Interpolated sound.
- Justifier support. Konami's Justifier is similar to the Super Scope and used 
  in Lethal Enforcers
...
If you only have a minimum level machine, you will need to stick to using only  
8-bit graphics and minimal or no sound. Disabling the joystick support will also
help.
...
Updated most recently by: funkyass
